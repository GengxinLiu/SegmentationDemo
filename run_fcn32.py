...
if mode == 'train':
    model = FCN32.build(height=256, width=256, num_classes=33)
    model = multi_gpu_model(model, gpus=2)
    model.summary()
    model.compile(optimizer=Adam(0.008),
    ...
    model.save_weights('pretrain/FCN32.h5')  # save model
    mode = 'test'
...
if mode == 'test':
    model = FCN32.build(height=256, width=256, num_classes=33)
    model = multi_gpu_model(model, gpus=2)
    model.load_weights('pretrain/FCN32.h5')
    num_images = 25
    images, categoricals = read_data(num_images=num_images)
    ...
    for i in range(num_images):
        mpimg.imsave('preds/FCN32/preds_{}.png'.format(i+1), annotations[i])
