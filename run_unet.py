...
if mode == 'train':
    model = UNET.build(height=256, width=256, num_classes=33)
    ...
    model.save_weights('pretrain/UNET.h5')  # save model
    mode = 'test'
...
if mode == 'test':
    model = UNET.build(height=256, width=256, num_classes=33)
    model = multi_gpu_model(model, gpus=2)
    model.load_weights('pretrain/UNET.h5')
    num_images = 25
    images, categoricals = read_data(num_images=num_images)
    ...
    for i in range(num_images):
        mpimg.imsave('preds/UNET/preds_{}.png'.format(i+1), annotations[i])
